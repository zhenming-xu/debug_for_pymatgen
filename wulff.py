...
class WulffShape:
    ...
    def get_plot(self, color_set='YlOrRd', grid_off=True, axis_off=True,
                 show_area=False, alpha=1, off_color='blue', direction=None,
                 bar_pos=(1.0,0.25,0.05,0.70), bar_on=True, units_in_JPERM2=True,
                 legend_on=True, aspect_ratio=(6, 6), custom_colors={}):
        """
        Get the Wulff shape plot.

        Args:
            color_set: default is 'YlOrRd'
            ...
            bar_pos: default is [1.0,0.25,0.05,0.70]
        ...
        if legend_on:
            ...
            if show_area:
                ax.legend(color_proxy, self.miller_area, loc='upper left', fontsize=18,
                          bbox_to_anchor=(0, 1), fancybox=True, shadow=False)
            else:
                ax.legend(color_proxy_on_wulff, miller_on_wulff,
                          loc='upper center',
                          bbox_to_anchor=(0.5, 1), ncol=3, fancybox=True, fontsize=18,
                          shadow=False)
        ax.set_xlabel('x')
        ax.set_ylabel('y')
        ax.set_zlabel('z')
