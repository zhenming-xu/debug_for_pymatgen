...
class DefectPhaseDiagram(MSONable):
    ...
    def plot(self, mu_elts=None, figsize=(7,9), xlim=None, ylim=None, ax_fontsize=1.3, lg_fontsize=1.,
             lg_position=None, tick_fontsize=1, fermi_level=None, title=None, saved=False):
        """
        ...
        for cnt, defnom in enumerate(xy.keys()):
            plt.plot(xy[defnom][0], xy[defnom][1], linewidth=2, color=colors[cnt])
        ...
        plt.axvline(x=0.0, linestyle='--', color='k', linewidth=1.5)  # black dashed lines for gap edges
        ...
        plt.xticks(size=tick_fontsize * width)
        plt.yticks(size=tick_fontsize * width)
        plt.xlabel("Fermi energy (eV)", size=ax_fontsize * width)
        plt.ylabel("Defect formation energy (eV)", size=ax_fontsize * width)
        if title:
            plt.title("{}".format(title), size=ax_fontsize * width)

        if saved:
            plt.savefig("Defect formation energy.png",dpi=800,bbox_inches='tight')
        else:
            return plt
